...
function weatherPanel(){simplePanel('🌤 '+t('weather'),`<input id="city" placeholder="${t('chooseCity')}"><button class="primary wide" id="weatherBtn">${t('search')}</button><div id="weatherResult" class="result"></div>`);weatherBtn.onclick=async()=>{weatherResult.textContent=t('working');try{const d=await api('/api/weather',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({city:city.value,language:lang})});weatherResult.innerHTML=richText(d.text)}catch(e){weatherResult.textContent=e.message}}}
...
function downloadPanel(){simplePanel('📥 '+t('download'),`<input id="downloadUrl" placeholder="${t('source')}"><select id="quality"><option value="360">360p</option><option value="720" selected>720p</option><option value="1080">1080p</option><option value="audio">MP3</option></select><button class="primary wide" onclick="doDownload()">${t('download')}</button><div id="downloadOut" class="result"></div>`)}async function doDownload(){downloadOut.textContent=t('working');try{const d=await api('/api/download',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:downloadUrl.value,quality:quality.value,user_id:tg?.initData||''})});downloadOut.innerHTML=`<b>${esc(d.title||'')}</b><br><a class="download" href="${d.url}" download>${d.audio?t('downloadMp3'):t('video')}</a>`}catch(e){const msg=String(e.message||e);downloadOut.innerHTML=e.status===429||/429|Too Many Requests|Render/.test(msg)?`<div class="error">⚠️ ${esc(msg)}</div>`:`<div class="error">❌ ${esc(msg)}</div>`}}
...
document.getElementById('lang').onclick=(event)=>{
 ...
 m.id='languageMenu';
 m.className='language-menu';
 m.setAttribute('role','menu');
 m.innerHTML=`
   <div class="language-title">🌐 ${esc(t('language'))}</div>
   <button type="button" role="menuitem" data-l="fa">🇮🇷 <span>${esc(t('fa'))}</span></button>
   <button type="button" role="menuitem" data-l="ckb">☀️ <span>${esc(t('ckb'))}</span></button>
   <button type="button" role="menuitem" data-l="en">🇬🇧 <span>${esc(t('en'))}</span></button>
 `;
 document.body.appendChild(m);
 requestAnimationFrame(()=>m.classList.add('show'));

 m.querySelectorAll('button[data-l]').forEach(button=>{
   button.addEventListener('click',(e)=>{
     e.preventDefault();
     e.stopPropagation();

     const selected=button.dataset.l;
     if(!selected) return;

     lang=selected;
     localStorage.setItem('asoland_lang',selected);
     ...
     // Reinitialize the complete UI so every page, panel and dynamic
     // component starts in the newly selected language.
     window.location.reload();
   });
 });

 const close=(e)=>{
   if(!m.contains(e.target) && e.target.id!=='lang'){
     m.classList.remove('show');
     setTimeout(()=>m.remove(),140);
