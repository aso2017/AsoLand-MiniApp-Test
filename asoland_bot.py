...
def _validate_netscape_cookies(raw: str, platform: str) -> str:
    ...
    if platform == "Instagram":
        missing = [k for k in ("sessionid", "csrftoken") if k not in names]
        if missing:
            return (
...
def _download_with_ytdlp(url, output_dir, platform, quality="720", progress_callback=None,
    ...
    """Primary yt-dlp downloader with YouTube fallback clients/formats."""
    ...
    base = {
        ...
        # Required by current YouTube extraction paths when Deno is available.
        "js_runtimes": {"deno": {}},
        "remote_components": ["ejs:github"],
    }

    if cookie_path and os.path.isfile(cookie_path) and os.path.getsize(cookie_path) > 100:
    ...
    if impersonate_target:
        try:
            ...
            base["impersonate"] = impersonate_target
        except Exception:
            pass
    ...
    if platform == "YouTube":
        ...
        if clients_env:
            client_sets = [[c.strip() for c in clients_env.split(",") if c.strip()]]
        else:
            # Same fallback order that proved successful in the working bot.
            client_sets = [None, ["android_vr"], ["web_safari"], ["android"]]
    else:
        client_sets = [None]
    ...
    for clients in client_sets:
        for strategy in strategies:
            ...
            if platform == "YouTube" and clients:
                youtube_args = {"player_client": clients}
                if po_token:
                    youtube_args["po_token"] = [f"{clients[0]}+{po_token}"]
                opts["extractor_args"] = {"youtube": youtube_args}
            elif platform == "YouTube":
                # Let yt-dlp choose the default client set for the first attempt.
                if po_token:
                    # PO tokens are client-specific; do not attach one to an
                    # unspecified client set.
                    pass
            ...
            try:
                ...
                if not os.path.exists(filename):
                    picked = _pick_downloaded_file(
                        output_dir, prefer_audio=(quality == "audio")
                    )
                    if picked:
                        filename = picked

                if not os.path.exists(filename) or os.path.getsize(filename) < 4096:
                    raise RuntimeError("فایل دانلودشده معتبر نیست")

                logger.info(
                    "YouTube download succeeded (client=%s, quality=%s)",
                    clients, quality
                )
                return {
                    "filename": filename,
                ...
                logger.debug(
                    "yt-dlp fallback failed (client=%s, quality=%s): %s",
                    clients, quality, exc
                )
                # Remove partial downloads before the next strategy.
                for candidate in glob.glob(os.path.join(output_dir, "*")):
                    try:
    ...
    raise RuntimeError(
        _friendly_download_error(
            RuntimeError(" | ".join(errors[-3:])[:1500]),
            platform
        )
    )
