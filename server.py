...
async def download(payload: DownloadIn):
    ...
    try:
        ...
        # Always serve relative to the job folder so /media/{sid}/name works
        # even if yt-dlp wrote an absolute path inside outdir.
        try:
            rel = path.relative_to(outdir)
        except ValueError:
            # Fallback: copy into outdir if somehow outside
        ...
        return {
            ...
            "audio": result.get("is_audio", False),
        ...
        raise HTTPException(400 if ("cookie" in message.lower() or "sessionid" in message.lower()) else 502, message)
